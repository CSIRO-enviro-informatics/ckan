...
echo "Restore" 
docker-compose    run --name $COMPOSE_NAME$RESTORE_NAME -e BYPASS_DB_INIT=true -e CONFIRM_RESTORE='Y' -e PGPASSWORD=$DATABASE_PASSWORD -e CKAN_BACKUP_FILE_NAME=$CKAN_BACKUP_FILE_NAME -e DATASTORE_BACKUP_FILE_NAME=$DATASTORE_BACKUP_FILE_NAME restore /restore.sh 
docker rm -f $COMPOSE_NAME$RESTORE_NAME
...
docker-compose    run --name $COMPOSE_NAME$SEARCH_NAME -e BYPASS_DB_INIT=true ckan ckan-paster --plugin=ckan search-index rebuild --config=/etc/ckan/default/ckan.ini
docker rm -f $COMPOSE_NAME$SEARCH_NAME
